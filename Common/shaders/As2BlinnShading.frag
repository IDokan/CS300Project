...
//File Name: As2BlinnShading.frag
...
vec3 CalculatePointLight(TheStruct item)
{
	...
	vec3 halfwayDir = normalize(lightVector + view);
	...
	vec3 lightSpecular = texture(specularTexture,outUv).rgb* pow(max(dot(vertexNormal, halfwayDir), 0), texture(specularTexture,outUv).r) * item.intensitySpecular;
...
vec3 CalculateSpotlightEffect(TheStruct item, float alpha)
{
	...
	vec3 lightAmbient = ambient * item.intensityAmbient;
	vec3 lightDiffuse = texture(diffuseTexture, outUv).rgb * max(dot(lightVector, vertexNormal), 0) * item.intensityDiffuse;
	vec3 lightSpecular = texture(specularTexture,outUv).rgb* pow(max(dot(vertexNormal, halfwayDir), 0), pow(texture(specularTexture,outUv).r, 2)) * item.intensitySpecular;
	lightSpecular.r = max(lightSpecular.r, 0);
	lightSpecular.g = max(lightSpecular.g, 0);
	lightSpecular.b = max(lightSpecular.b, 0);
